                ...
                <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                    ...
                    @foreach($lords as $lord)
                        <tr>
                            ...
                            <td>{{ $lord->name }}</td>
                            <td>{{ $lord->description }}</td>
                            <td>{{ $lord->category }}</td>
                            <td><img src="{{ Storage::url($lord->image) }}"
                                     class="img-thumbnail" height="50px" width="100px"
                                     alt="Lord"></td>
                            <td>{{ $lord->slug }}</td>
                            <td><!-- Button trigger modal -->
                                <button type="button" class="btn btn-outline-warning btn-sm" data-toggle="modal"
                                        data-target="#modelId{{ $lord->id }}">
                                    Update
                                </button>

                                <!-- Modal -->
                                <div class="modal fade" id="modelId{{ $lord->id }}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                ...
                                                <h4 class="modal-title" id="modelTitleId">Update {{ $lord->name }}</h4>
                                            </div>
                                            <div class="modal-body">
                                                <form action="/updateProduct/{{ $lord->id }}" method="post" enctype="multipart/form-data" class="form-horizontal">
                                                    @csrf
                            ...
                            <td><a id="deletebtn" class="btn btn-outline-danger btn-sm" href="/deleteLord/{{ $lord->id }}" role="button">Delete</a></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
