    ...
    @section('content')
        ...
        @foreach($complaints as $complaint)
            <div class="panel panel-info" style="margin: 10px; padding: 10px;">
                ...
                @if(count($comments)>0)
                    @php $commentos = $comments->where('complaint',$complaint->id)->get(); @endphp
                @if(count($commentos)>0)
                    @foreach($commentos as $comment)
                <div class="list-group">
                    <a href="#" class="list-group-item" style="margin: 10px; margin-left: 10px">
                        <h4 class="list-group-item-heading"><b style="font-family: 'Bookman Old Style'">{{ DB::table('users')->where('user', $comment->user)->first()->name }}</b> has commented:</h4>
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h4 class="modal-title" id="modelTitleId">Reply to {{ DB::table('users')->where('user', $comment->user)->first()->name }}</h4>
                                    </div>
                ...
                <div class="list-group">
                    <a href="#" class="list-group-item" style="margin: 10px; margin-left: 10px">
                        <h4 class="list-group-item-heading">Add a comment:</h4><form>
                            <textarea name="" id=""  rows="10" placeholder="Add a Comment" style="width: 100%"></textarea>
                        </form>
                    </a>
                </div>
            </div>
